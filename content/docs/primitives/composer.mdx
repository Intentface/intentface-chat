...
(`composer-command-group-label`). To group, read the resolved items with
`useCommandListItems()`, partition them by your own field, and render a
`CommandGroup` + `CommandGroupLabel` per bucket, mapping the bucket's items to
`CommandItem`s — keyboard nav still flows top-to-bottom because the resolved order
is store-driven, not derived from the render tree.
