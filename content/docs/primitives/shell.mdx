...
- **No global keys** — the package claims none; bind your own around `toggle`. Only the grip's arrow keys are local enough to belong here.
...
- **Hover-peek** — a collapsed sidebar floats back out when the pointer rests against the screen edge. Render `Shell.Hotspot` to opt in; omit it to opt out.
...
`Shell.Grip` does own its keys, because those are local rather than
global: with the grip focused, the arrow keys nudge the width by `step`.
...
stylesheet — the grip writes `--shell-sidebar-width` and your rule
...
relationship are both announced. `Shell.Grip` is a focusable
...
`Shell.Hotspot` is `aria-hidden`: it is a pointer affordance with no keyboard
...
The drag affordance. Renders `data-shell-grip` as a focusable
...
Renders `data-shell-hotspot`, `aria-hidden`. Give it a width and a position
