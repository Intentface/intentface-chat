...
— lives in a store. Every `<Composer>` creates its own isolated instance, so a
...
It throws outside a `<Composer>` — inside the tree, no store is ever named:
...
`<Composer>`, hooks resolve by context and nothing is named; outside it,
