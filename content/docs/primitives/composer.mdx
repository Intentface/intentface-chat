...
Renders `data-slot="composer-panel"`. The matched child renders directly; for
open/close transitions, compose motion through the standard `render` prop —
`open` comes from the panel's state, the matched child arrives as
`props.children`, and `data-open`/`data-closed` are exposed to style against.
