...
const DesktopPanel = () => {
  const { activeArtifact, isArtifactOpen: isOpen, closeArtifact: closePanel } = useChatContext();

  if (!activeArtifact) return null;

  return (
    <ArtifactsPanel open={isOpen}>
      <ArtifactsPanel.Header title={activeArtifact.title} onClose={closePanel} />
      <ArtifactsPanel.Viewport>
        <ArtifactsPanel.Content>{activeArtifact.content}</ArtifactsPanel.Content>
      </ArtifactsPanel.Viewport>
  ...
  return (
    <Drawer side="right" open={isOpen} onOpenChange={closePanel}>
      <Drawer.Content side="right" className="w-full sm:max-w-lg">
        ...
        {activeArtifact && <ArtifactsPanel.Footer content={activeArtifact.content} />}
      </Drawer.Content>
    </Drawer>
  );
};
