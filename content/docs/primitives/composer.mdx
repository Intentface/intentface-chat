...
type ComposerSubmitData =
  | { kind: "message"; text: string; files: FilePart[] }
...
export const attachmentsProps = [
  { name: "accept", type: "string", default: "DEFAULT_ATTACHMENT_ACCEPT", description: "Accepted MIME types." },
  { name: "maxFiles", type: "number", default: "DEFAULT_ATTACHMENT_MAX_FILES", description: "Maximum file count." },
  { name: "maxFileSize", type: "number", default: "DEFAULT_ATTACHMENT_MAX_FILE_SIZE", description: "Maximum size per file." },
...
Shows the `PanelItem` whose `value` matches, and force-routes to the
command-list item while a command prefix is open. Renders
...
export const panelAttrs = [
  ...
  { attribute: "data-open / data-closed", description: "Reflects whether a panel item is open." },
];
...
export const panelItemProps = [
  { name: "value", type: "string", default: "(required)", description: "Match key against Panel's value or the \"command-list\" override." },
];
...
`Composer.CommandLoading` (`composer-command-loading`, default "Loading…"),
`Composer.CommandEmpty` (`composer-command-empty`, default "No results
found"), and `Composer.CommandDismiss` (`composer-command-dismiss`, a button)
fill the list states. `Composer.CommandGroup` renders
...
`Composer.AskUserHints` renders keyboard hints (defaults to ↑↓ / ↵ / ←→ / esc).
`Composer.AskUserDismiss` is a button (default label "Dismiss",
`data-slot="composer-ask-user-dismiss"`). `Composer.AskUserContinue` is a
submit button (`data-slot="composer-ask-user-continue"`) whose default label is
"Continue", or "Submit" on the last question.
