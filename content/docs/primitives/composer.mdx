---
title: Composer
description: Rich-text chat input with chips, slash/mention commands, attachments, and a request flow (questions and tool approvals).
source: composer
---
...
import { ApprovalFlow } from './composer/demos/approval-flow';
import { AskFlow } from './composer/demos/ask-flow';
...
- **Panel** — hosts command results, live steps, or a request prompt above the field.
...
the state it belongs to (`commands.active`, `requests.active`, …) and the panel
...
Setting the `requests` prop — a clarifying question, a tool approval, any
choice you put to the user — arms the request flow and flips `requests.active`;
compose the `Ask` parts from `@intentface/chat/ask` inside a `Panel` (or
`Popover`), reading the current step from `useComposer(c => c.requests)`. The
flow steps through each question (single- or multi-select), and answering or
skipping the last one fires `onSubmit` with `{ kind: "requests" }` — one flat
entry per request, keyed by the id you minted, carrying option values. Passing
a fresh `requests` array re-arms it from the first step.

<Demo component={<AskFlow />} file="primitives/composer/demos/ask-flow.tsx" />

A tool approval is the same mechanism, not a feature: mint the tool call id as
the request id, put your decision vocabulary in the option `value`s, and route
the entry back to the tool in `onSubmit`. A single request mounts no
step-navigation parts.

<Demo component={<ApprovalFlow />} file="primitives/composer/demos/approval-flow.tsx" />
...
`derivePanelState` is yours — what belongs in the panel (a request prompt, a
...
the request flow being active, takes priority over normal typing.
...
<AttributesTable rows={requestKeys} />
...
While `requests` are active, the options form a labelled group: `Ask.Options`
renders `role="radiogroup"` (single-select) or `role="group"` (multi-select),
named by `Ask.Label` and described by `Ask.StepLabel` automatically.
Each `Ask.Option` is the real control — `role="radio"` / `role="checkbox"`
...
- **Request focus.** Activating `requests` moves focus into the options
...
export const rootProps = [
  { name: "onSubmit", type: "(data: ComposerSubmitData) => void | Promise<void>", description: "Fires on message submit and on request entries. Discriminate on data.kind." },
  ...
  { name: "requests", type: "ComposerRequest[]", description: "When set, arms the request flow (blurs the editor, flips requests.active)." },
...
type ComposerSubmitData =
  ...
  | { kind: "requests"; requests: ComposerRequestEntry[] };
```
...
by priority (`commands.active ? <Command/> : requests.active ? <Ask.Root/> :
...
The composer owns the request *state* — `items`, the current step, the
drafts — but renders none of the prompt UI. Compose that from the `Ask`
namespace in `@intentface/chat/ask`, reading the step through
`useComposer((c) => c.requests)` and gating the enclosing `Panel` or `Popover`
on `requests.active`.

`Ask.Dismiss` is a plain button you wire to `requests.dismissStep`;
`Ask.Continue` is `type="submit"`, so the enclosing `Composer.Root` form
drives it. Neither ships copy — supply the labels as children, and read
`requests.isLastStep` to switch the continue wording.
