...
- **Selection → chat** — `Message.SelectionToolbar` lifts highlighted text back into the composer.
...
The root: role-aware container. Renders `data-slot="message"`.

export const rootProps = [
  { name: "role", type: '"system" | "user" | "assistant"', default: "(required)", description: "Drives role-based styling via data-role." },
...
export const rootAttrs = [
  { attribute: "data-slot", values: '"message"', description: "The message root." },
  { attribute: "data-role", values: '"system" | "user" | "assistant"', description: "The message role." },
...
Groups a role's messages into one visual turn. Renders `data-slot="message-turn"`.
...
A source pill linking out. Renders `data-slot="message-source"` as
...
Formatted send time. Renders `data-slot="message-timestamp"`.

export const timestampProps = [
  { name: "timestamp", type: "Date | string | number", default: "(required)", description: "The time to format (children override the formatted value)." },
];

<PropsTable rows={timestampProps} />
...
export const attachmentProps = [
  { name: "attachment", type: "FilePart", default: "(required)", description: "The attachment to render." },
];

<PropsTable rows={attachmentProps} />

### Message.SelectionToolbar
...
`Message.Error`, `Message.Stopped`, and `Message.Loading` are layout/marker
slots taking only `children`; they render `data-slot="message-content"`,
`"message-actions"`, `"message-attachments"`, `"message-sources"`,
`"message-error"`, `"message-stopped"`, and `"message-loading"` respectively.
